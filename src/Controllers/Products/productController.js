...
productController.fetchProductsToDb = async () => {
  try {
    ...
    const getProducts = await axios(
      "https://fashionapi.up.railway.app/all/blsmgfdk2512196430032000"
    );
...
productController.filterProducts = async (fields) => {
  ...
  try {
    ...
    if (color) {
      if (filteredProducts.length) {
        const filter = filteredProducts.filter((el) =>
    ...
    if (priceMin && priceMax) {
      if (filteredProducts.length) {
        const filter = filteredProducts.filter(
          (el) => Number(el.price) > priceMin && Number(el.price) < priceMax
        );
