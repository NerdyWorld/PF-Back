...
userHandler.adminLoginHandler = async (req, res) => {
  const response = await userController.adminLogin(req.body);

  if (
    response.msg === "Wrong Email" ||
    response.msg === "Wrong Username" ||
    response.msg === "Invalid credentials for admin"
  ) {
    res.status(403).json(response);
  } else {
    res.status(200).json(response);
...
userHandler.googleAuthHandlerAdmin = async (req, res) => {
  const response = await userController.googleAuthAdmin(req.body);

  if (
    response.msg === "Account already associated with Google Email" ||
    response.msg === "Invalid credentials for admin"
  ) {
...
userHandler.githubAuthHandlerAdmin = async (req, res) => {
  const response = await userController.githubAuthAdmin(req.params.gitCode);
